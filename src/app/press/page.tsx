...
function PressPackLink (props: { pack: PressPack }): JSX.Element {
    ...
    return (
        <Link href={link} className="p-5">
            <p className="text-orange underline decoration-black text-xl"> { title } </p>
        </Link>
    )
}

async function getPressArticles(): Promise<PressCoverage[]> {
  const getArticles = apiClient.path("/api/v1/press_coverage").method("get").create();
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap flex-col items-center justify-center sm:flex-row">
        {pressPacks.map(pack => <PressPackLink key={pack.title} pack={pack}/>)}
      </div>
    </div>
  );
};
